---
title: "High-frequency disaggregation"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{High-frequency disaggregation (e.g., to daily)}
...
It is no longer restricted to regular conversions, where each low frequency period has the same number of high frequency periods.
...
Thus, for high-frequencies, the computation may take a long time.
...
The raw series are included as examples in the package.
...
Because we are dealing with daily data, we keep the data in `data.frame`, rather than `ts` objects. Other time series objects, such as `xts` and `tsibble`, are possible as well.
...
Like with all disaggregation methods in tempdisagg, the resulting series fulfills the disaggregation constraint:
